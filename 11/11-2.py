...
universe_expansion_speed = 999_999
...
for galaxy in galaxies:
    for path in galaxy[1]:
        path_length = 0
        for expanding_row in empty_rows:
            # print(f"Checking if row {expanding_row} is between {galaxy[0]} and {path}")
            if (expanding_row > galaxy[0][1] and expanding_row < path[1]) or (expanding_row > path[1] and expanding_row < galaxy[0][1]):
                # print("IT IS!")
                path_length += universe_expansion_speed
        for expanding_column in empty_columns:
            # print(f"Checking if column {expanding_column} is between {galaxy[0]} and {path}")
            if (expanding_column > galaxy[0][0] and expanding_column < path[0]) or (expanding_column > path[0] and expanding_column < galaxy[0][0]):
                # print("IT IS!")
                path_length += universe_expansion_speed
        path_length_sum += abs(galaxy[0][0] - path[0]) + abs(galaxy[0][1] - path[1]) + path_length
